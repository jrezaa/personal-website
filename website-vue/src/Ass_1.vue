...
const myAge = 27;
let myName = ref('Jeff');
const ageInFive = (age: number): number => {
  return age + 5;
};
const getRandomNumber = computed((): number => Math.random());
const setName = ($event: Event) => {
  myName.value = ($event.target as HTMLInputElement).value;
};
const imgUrl =
    ...
    <div>
      ...
      <div class="pl-5">
        ...
        <section id="assignment" class="pb-10">
          ...
          <input type="text" :value="myName" class="text-black" @input="setName($event)" />
        </section>
      </div>
    </div>
  </body>
